...
function updateSelectedCount() {
  // 전체 선택 체크박스를 제외한 선택된 항목만 계산
  const selectedCheckboxes = document.querySelectorAll(
    ".custom__checkbox:checked:not(#custom__box)"
  ).length;

  // 선택된 항목 개수 표시 (최소 0 보장)
  if (itemCountElement) {
    itemCountElement.innerText = `(${selectedCheckboxes})`;
  }
}
...
function addEventListeners() {
  document.querySelectorAll(".original-add").forEach(function (button) {
    button.addEventListener("click", function (event) {
      ...
      if (index >= 0) {
        if (getCartProducts[index].quan < 5) {
          // 최대 수량 제한 조건
